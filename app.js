...
function renderTalents(){
 ...
 $("talentBranches").innerHTML=branches.map(branch=>{const tiers=[1,2,3,4,5].map(tier=>{const nodes=TALENTS.filter(t=>t[1]===branch.code&&t[4]===tier).sort((a,b)=>a[5]-b[5]);if(!nodes.length)return"";return '<div class="talent-tier"><div class="talent-tier-label">Уровень '+tier+'</div><div class="talent-tier-grid">'+nodes.map(t=>{const rank=talentRank(t[0]),unlocked=t[9].every(req=>talentRank(req)>=5),up=canUpgrade(t),req=t[9].length?"Требует: "+t[9].map(code=>talentDef(code)[2]).join(", "):"Начальный талант";return '<article class="talent-node '+(rank?"has-rank ":"")+(unlocked?"unlocked":"locked")+'" data-talent="'+t[0]+'"><div class="talent-node-top"><span>'+t[0].replace("TALENT_","")+'</span><b>'+rank+'/5</b></div><h3>'+t[2]+'</h3><p>'+t[3]+'</p><div class="talent-effect">'+talentEffectText(t)+'</div><div class="talent-requirement">'+req+'</div><div class="talent-actions">'+(rank?'<button type="button" data-talent-down="'+t[0]+'">−</button>':"")+'<button type="button" class="talent-up" data-talent-up="'+t[0]+'" '+(up?"":"disabled")+'>'+(rank>=5?"Макс.":rank?"+1":"Изучить")+'</button></div></article>'}).join("")+'</div></div>'}).join("");return '<section class="talent-branch"><header><div><h3>'+branch.name+'</h3><p>'+branch.desc+'</p></div><span>'+spentTalentPoints()+' очков</span></header>'+tiers+'</section>'}).join("");
...
document.addEventListener("click",e=>{
 ...
 const down=e.target.closest("[data-talent-down]");if(down){const t=talentDef(down.dataset.talentDown),rank=talentRank(down.dataset.talentDown);if(t&&rank>0&&canDowngrade(t)){state.talents[t[0]]=rank-1;saveState();render();openTalents()}return}
